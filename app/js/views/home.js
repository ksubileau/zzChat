...
define(['jquery', 'backbone', 'underscore', 'i18next', 'views/disposable', 'models/user', 'text!templates/home.html', 'jquery.placeholder'],
    function($, Backbone, _, i18n, DisposableView, UserModel, homeView){
        ...
        var HomeView = DisposableView.extend({
            ...
            initialize: function() {
                if(zzChat.options.enableRememberMe && localStorage && localStorage.lastUser) {
                    try {
                        var lastUser = JSON.parse(localStorage.lastUser);
                        this.lastUser = new UserModel(lastUser);
                    } catch (e) { }
                }
            ...
            render: function() {
                ...
                this.$el.html(this.template({
                    ...
                    langList: _.reject(zzChat.options.langAvailable, function(lang) {
                        return lang.langcode === zzChat.getLanguage();
                    }),
                ...
                if(this.lastUser) {
                    // Restore last values
                    this.$('#nickname').val(this.lastUser.get('nick'));
                    this.$('#age').val(this.lastUser.get('age'));
                    if(this.lastUser.get('sex') === 2 || this.lastUser.get('sex') === 'female') {
            ...
            login : function(e){
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        else if (localStorage && localStorage.lastUser) {
                            // Clear previous data
